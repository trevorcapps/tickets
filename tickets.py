...
parser = argparse.ArgumentParser(description="ServiceNow Vulnerability Ticket Management TUI")
...
TABLE_NAME = SNOW_API_ENDPOINT.rsplit('/', 1)[-1] if SNOW_API_ENDPOINT else ''
SNOW_HEADERS = {
...
def today_str():
    """Return today's date string (computed at call time, not import time)."""
...
def map_severity(sev_priority):
    """Map severity_priority to ServiceNOW severity label and due-date offset (days)."""
    return {1: ('Critical', 30), 2: ('High', 60), 3: ('Medium', 90)}.get(sev_priority, ('Low', 120))


def fetch_assignment_groups():
    """Fetch and cache assignment group name -> sys_id mapping from ServiceNow."""
    group_mapping = {}
    url = f"{SNOW_INSTANCE_URL}/api/now/table/sys_user_group"
...
def fetch_ticket_data():
    ...
    """
    logger.debug("Executing SQL query for ticket data")
...
def fetch_ticket_data_by_cve(cve_list):
    ...
    query = f"""
        ...
        GROUP BY remediation_summary, assignment_group
    """
    logger.debug("Executing SQL query for ticket data by CVE")
    with get_db_connection() as conn:
        with conn.cursor() as cur:
...
def close_snow_ticket(sn_sys_id):
    payload = {"state": SNOW_STATE_CLOSED}
    url = f"{SNOW_API_ENDPOINT}/{sn_sys_id}"
...
def insert_ticket_tracking(sn_ticket_id, sn_sys_id, summary, assignment_group, findings, severity_label):
    ...
    try:
        ...
        logger.error("Failed to insert tracking for ticket %s: %s", sn_ticket_id, e)
...
def update_ticket_tracking(sn_sys_id, short_description, status, assignment_group, management_comments):
    try:
        ...
        logger.error("Failed to update tracking for sys_id %s: %s", sn_sys_id, e)
...
def close_local_ticket(sn_sys_id):
    try:
        ...
        logger.error("Failed to close local ticket %s: %s", sn_sys_id, e)


# --- Shared Ticket Creation Logic ---

def _format_description(row, include_cve=False):
    """Build the ticket description from a data row."""
    description = (
...
def _show_debug_payloads(data):
    """Print full JSON payloads in debug mode."""
...
def _show_summary_table(data, title="Ticket Creation Summary"):
    """Display a summary table grouped by assignment group."""
...
def _confirm_proceed():
    """Ask user to confirm. Returns True if they want to proceed."""
    return Prompt.ask("Proceed with ticket creation? (y/n)", choices=["y", "n"], default="n") == "y"


def _process_ticket_creation(data, include_cve=False):
    """Shared logic for creating tickets from aggregated data rows."""
    if not data:
    ...
    if DEBUG_MODE:
        _show_debug_payloads(data)
    else:
        title = "CVE-Based Ticket Creation Summary" if include_cve else "Ticket Creation Summary"
        _show_summary_table(data, title=title)
    ...
    for row in data:
        try:
            ...
            summary = row[0]
            description = _format_description(row, include_cve=include_cve)
            ...
            logger.error("Failed to create ticket for '%s': %s", row[0], e)
            console.print(f"[red]Error creating ticket for '{row[0]}': {e}[/red]")
...
def create_tickets():
    ...
    _process_ticket_creation(data, include_cve=False)
...
def create_tickets_by_cve():
    ...
    _process_ticket_creation(data, include_cve=True)
...
def ticket_sync():
    ...
    for sn_sys_id in sn_sys_ids:
        try:
            ...
            logger.error("Failed to sync ticket %s: %s", sn_sys_id, e)
            console.print(f"[red]Error syncing ticket {sn_sys_id}: {e}[/red]")
...
def append_tickets():
    ...
    if not tickets_dict:
        console.print("[green]No findings to append.[/green]")
        Prompt.ask("Press Enter to return to menu")
        return
    ...
    if DEBUG_MODE:
        ...
        for sn_ticket_id, findings in tickets_dict.items():
            ...
            for f in findings:
                comment += (
                    f"- Ticket ID: {sn_ticket_id} | Host: {f['host']} | Port: {f['port']} | Severity: {f['dlx_severity']} | "
    ...
    for sn_ticket_id, findings in tickets_dict.items():
        try:
            ...
            logger.error("Failed to append to ticket %s: %s", sn_ticket_id, e)
            console.print(f"[red]Error appending to ticket {sn_ticket_id}: {e}[/red]")
    ...
    if Prompt.ask("Proceed with closing these tickets? (y/n)", choices=["y", "n"]) == "y":
        for sn_ticket_id, sn_sys_id in tickets_to_close:
            try:
                ...
                logger.error("Failed to close ticket %s: %s", sn_ticket_id, e)
                console.print(f"[red]Error closing ticket {sn_ticket_id}: {e}[/red]")
...
def fetch_attachment_data():
    query = """SELECT sn_ticket_id, sn_sys_id, asset_name, ip_address, operating_system,
                      environment, managed_by, proof, assignment_group, pci_scope,
                      dlx_category, dlx_severity, title, port, first_found_date,
                      last_scan_date, remediation_summary, remediation, status
               FROM update_ticket_attachment
               ORDER BY sn_ticket_id"""
    logger.debug("Executing fetch_attachment_data SQL")
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(query)
            return cur.fetchall()
...
def add_csv_attachment(sys_id, csv_content, filename):
    ...
    resp = requests.post(url, auth=SNOW_AUTH, headers=SNOW_HEADERS, params=params, files=files)
    try:
        resp.raise_for_status()
    except requests.HTTPError:
        logger.error("Failed to upload attachment %s to ticket %s: %s %s",
...
def update_ticket_attachments():
    ...
    for ticket_id, entries in tickets.items():
        try:
            ...
            headers = ['sn_sys_id', 'asset_name', 'ip_address', 'operating_system', 'environment',
                       'managed_by', 'proof', 'assignment_group', 'pci_scope', 'dlx_category',
                       'dlx_severity', 'title', 'port', 'first_found_date', 'last_scan_date',
                       'remediation_summary', 'remediation', 'status']
            writer.writerow(headers)
            for entry in entries:
                writer.writerow(entry)

            csv_data = output.getvalue().encode('utf-8')
            filename = f"{ticket_id}_{today_str()}.csv"
            add_csv_attachment(sys_id, csv_data, filename)
        except Exception as e:
            logger.error("Failed to update attachments for ticket %s: %s", ticket_id, e)
            console.print(f"[red]Error updating attachments for {ticket_id}: {e}[/red]")
...
def handle_exceptions():
    ...
    while True:
        ...
        try:
            ...
            logger.error("Failed to review ticket %s: %s", ticket_id, e)
            console.print(f"[red]Error reviewing ticket {ticket_id}: {e}[/red]")


# --- Append Tickets Menu Item ---

def append_tickets_menu():
    append_tickets()
...
def main():
    while True:
        ...
        console.print("5. Append Ticket Comments")
        console.print("6. Close Tickets")
        console.print("7. Ticket Sync")
        console.print("8. Handle Exceptions")
